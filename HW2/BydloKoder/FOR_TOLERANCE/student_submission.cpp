...
int start_for_all_x = 8;
int original_x = start_for_all_x;
int start_for_all_y = 7; 
const int parallel_portion_y = 17;
const int parallel_portion_x = 25;
...
void check_vertical_split(int i_or, int j_or, bool checked[parallel_portion_y][parallel_portion_x],bool all, int i, int j, int len_boundary_x, int len_boundary_y, int max_iter, double power, double view_x0, double view_y1, double x_stepsize, double y_stepsize);
...
bool check_boundary_y(int i_or, int j_or, bool checked[parallel_portion_y][parallel_portion_x],bool all, bool &allfalse, int i, int j, int len_boundary_y, int max_iter, double power, double view_x0, double view_y1, double x_stepsize, double y_stepsize){
	...
	for(int l = i; l < i + len_boundary_y; l++){
		...
		if (!current){
			all = false;
			checked[i%i_or][j%j_or] = false;
		}	
		else{
			allfalse = false;
			checked[i%i_or][j%j_or] = true;
		}
	}
	return all;
}

bool check_boundary_x(int i_or, int j_or, bool checked[parallel_portion_y][parallel_portion_x], bool all, bool &allfalse, int i, int j, int len_boundary_x, int max_iter, double power, double view_x0, double view_y1, double x_stepsize, double y_stepsize){
	//std::cout << len_boundary_x << " " << len_boundary_y << std::endl;
	double y;
	double x;
	bool current;
...
bool check_checked(int i_or, int j_or, bool all, bool checked[parallel_portion_y][parallel_portion_x], int i, int j, int len_boundary_x, int len_boundary_y){
	i = i%i_or;
	j = j%j_or;
	if(!all)
		return false;
	for(int l = i; l < i + len_boundary_y; l++){
		if (!checked[l][j] or !checked[l][j+len_boundary_x-1])
			return false;
	}
	for(int l = j; l < j + len_boundary_x; l++){
		if (!checked[i][l] or !checked[i+len_boundary_y-1][l])
			return false;
	}
	return true;
}

void check_horizontal_split(int i_or, int j_or, bool checked[parallel_portion_y][parallel_portion_x], bool all, int i, int j, int len_boundary_x, int len_boundary_y, int max_iter, double power, double view_x0, double view_y1, double x_stepsize, double y_stepsize){
	if (!check_checked(i_or, j_or, all, checked, i, j, len_boundary_x, len_boundary_y)){
		if (len_boundary_x == 7 and len_boundary_y == 5){
	//std::cout << "pos " << i << ' ' << j << std::endl;
	//std::cout << "lens " << len_boundary_y << ' ' << len_boundary_x << std::endl;
			double x;
			double y;
			for(int l = i + 1; l < i + len_boundary_y - 1; l++){
				for(int m = j + 1; m < j + len_boundary_x - 1; m++){
		...
		bool allfalse = true;
		all = check_boundary_y(i_or, j_or, checked, true, allfalse, i+1, j + len_boundary_x/2, len_boundary_y-2, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
		check_vertical_split(i_or, j_or, checked, all, i, j, len_boundary_x/2 + 1, len_boundary_y, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
		//std::cout << "between left and right" << std::endl;
		check_vertical_split(i_or, j_or, checked, all, i, j + len_boundary_x/2, len_boundary_x/2+1, len_boundary_y, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize); 
	}
	else{
		pthread_mutex_lock(&mutex_count);
		pointsInSetCount += 2 * (len_boundary_x-2)*(len_boundary_y-2);
		pthread_mutex_unlock(&mutex_count);
	}
...
void check_vertical_split(int i_or, int j_or, bool checked[parallel_portion_y][parallel_portion_x],bool all, int i, int j, int len_boundary_x, int len_boundary_y, int max_iter, double power, double view_x0, double view_y1, double x_stepsize, double y_stepsize){
	...
	//std::cout << "lens " << len_boundary_y << ' ' << len_boundary_x << std::endl;
		...
		bool allfalse = true;
		all = check_boundary_x(i_or, j_or, checked, true, allfalse, i + len_boundary_y/2, j+1, len_boundary_x - 2, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
		check_horizontal_split(i_or, j_or, checked, all, i, j, len_boundary_x, len_boundary_y/2 + 1, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
		//std::cout << "between up and down" << std::endl;
		check_horizontal_split(i_or, j_or, checked, all, i + len_boundary_y/2, j, len_boundary_x, len_boundary_y/2 + 1, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize); 
	}
	else{
		pthread_mutex_lock(&mutex_count);
		pointsInSetCount += 2 * (len_boundary_x-2)*(len_boundary_y-2);
...
void check_square(int i, int j, int max_iter, double power, double view_x0, double view_y1, double x_stepsize, double y_stepsize)
{
	bool checked[parallel_portion_y][parallel_portion_x];
	int len_boundary_x = parallel_portion_x;
	int len_boundary_y = parallel_portion_y;
	bool all = true;
	bool allfalse = true;
	all = check_boundary_y(i, j, checked, all, allfalse, i, j, len_boundary_y, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
	all = check_boundary_y(i, j, checked, all, allfalse,  i, j+len_boundary_x-1, len_boundary_y, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
	all = check_boundary_x(i, j, checked, all, allfalse, i, j+1 ,len_boundary_x - 2, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
	all = check_boundary_x(i, j, checked, all, allfalse, i+len_boundary_y-1, j+1 ,len_boundary_x - 2, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
	...
	if (all){
		...
		all = check_boundary_y(i, j, checked, true, allfalse, i+1, j + len_boundary_x/2, len_boundary_y-2, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
		check_vertical_split(i, j, checked, all, i, j, len_boundary_x/2 + 1, len_boundary_y, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
		//std::cout << "HERE HERE HEHE REHEHEHEHEHEHEH";
		check_vertical_split(i, j, checked, all, i, j + len_boundary_x/2, len_boundary_x/2+1, len_boundary_y, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize); 
	}
	//std::cout << all;
}
...
void* parallel_part(void* args_set){
	...
	int x_resolution = args->x_res - 2 ; // temporary TODO remove
	...
	while(true){
		...
		check_square(i, j, max_iter, power, view_x0, view_y1, x_stepsize, y_stepsize);
	}
